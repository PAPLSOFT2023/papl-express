...
@Component({
  ...
  styleUrls: ['./existing-report.component.scss']
})
export class ExistingReportComponent {
  ...
  fetchUploadFiles() {
    ...
    this.http.get<any[]>(`${environment.serverUrl}api/upload_report_fetch1?name=${value}`).subscribe(data => {      this.uploadFiles = data;
