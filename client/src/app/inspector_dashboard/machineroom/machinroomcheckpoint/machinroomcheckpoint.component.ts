...
// async savemach(): Promise<void> {
//   console.log('checkpoints are',this.checkpoint);
//   console.log('risk level array',this.risk_level);
...
//   console.log('checked array of machine room',this.checkpoint);
//   console.log('risk_level+',this.risk_level);

//   for (let i = 0; i < this.risk_level.length; i++) {
//     if (this.risk_level[i] === '5' && this.checkpoint[i]) {
//       console.log('machine room logic called');
...
//   try {
//     const result = await this.apicallservice.insert_Record_Values("from mach",
//       this.id_no,
//       this.documentId, 
//       this.inspectorName,
//       this.section,
        ...
        if ('indexedDB' in window) {
          ...
          this.saveDataLocally(valueArray,this.Negative_ADJ,this.Negative_MNT).then(() => {
            ...
            this.showAlert("Data saved to OutBox").then(() => {
              this.router.navigate(['afterlogin', 'machineroom', this.section]);
            });
          }).catch((error) => {
            console.error("Error saving data locally:", error);
          });
